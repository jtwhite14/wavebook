...
export function IncomingInvites({ invites, onResolved }: IncomingInvitesProps) {
  ...
  return (
    <div className="rounded-lg border border-primary/30 bg-background/95 backdrop-blur-sm shadow-[--shadow-popover] p-4">
      <p className="text-sm font-medium tracking-[-0.01em] mb-3">
        {invites.length === 1 ? "1 spot shared with you" : `${invites.length} spots shared with you`}
      </p>
      <div className="space-y-2">
        {invites.map((invite) => (
          <div key={invite.id} className="flex items-center gap-3 rounded-lg border px-3 py-2">
